    ...
    <section className="space-y-3 rounded-xl bg-white p-6 shadow">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {sanitizedImages.map((image, index) => (
          <button
            type="button"
            key={`${image}-${index}`}
            className="relative aspect-[4/3] overflow-hidden rounded-lg bg-slate-200 text-left"
      ...
      {activeIndex !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 p-4">
          <button className="absolute right-4 top-4 text-white" onClick={close} type="button">
            Close ✕
          </button>
          <button className="absolute left-4 rounded bg-white/20 px-3 py-2 text-white" onClick={previous} type="button">
            ‹
          </button>
          ...
          <button className="absolute right-4 rounded bg-white/20 px-3 py-2 text-white" onClick={next} type="button">
            ›
          </button>
        </div>
      )}
