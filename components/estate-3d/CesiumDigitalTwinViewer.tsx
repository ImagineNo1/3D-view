...
export function CesiumDigitalTwinViewer({ config, onFallback }: Props) {
      ...
      try {
        ...
        if (config.modelUrl) {
          try {
            ...
            setNote(t.viewer.realWorldNote);
          }
        } else {
          setNote(t.viewer.realWorldNote);
        }
  ...
  }, [config, onFallback, t.viewer.missingMapKey, t.viewer.realWorldNote]);
  ...
  return (
    <div className="relative min-h-[620px] overflow-hidden rounded-3xl bg-slate-950 text-white" dir={locale === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="absolute left-3 top-3 z-10 flex flex-wrap gap-2">
        <span className="rounded-full bg-slate-900/85 px-3 py-2 text-xs font-semibold shadow-lg ring-1 ring-white/10">{t.viewer.realWorldMode}</span>
